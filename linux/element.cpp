...
namespace element
{
    ...
    void Element::draw(int width, int height, int font_size)
    {
        ...
        py::object format_style_object = root_element["format_style"].attr("style");
        if (tag == "text")
        {
            std::string text = root_element["text"].cast<std::string>();

            std::tuple<int, int> pos = returnTextPosition(width, height, font_size, text, format_style_object, cr);
    ...
    std::tuple<int, int> Element::returnTextPosition(int width, int height, int font_size, std::string text, py::object format_style, cairo_t* cr)
    {
        std::tuple<int, int> pos;
        if (!format_style.is_none()) {
            cairo_text_extents_t text_extents;
            cairo_text_extents(cr, text.c_str(), &text_extents);
            pos = style::Style::handleFormatStyle(width, height, font_size, text_extents.width, text_extents.height, format_style);
        }
        else
        {
