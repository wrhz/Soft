...
def build_linux(object_dir):
    try:
        x86_64_path = os.path.join(object_dir, "build", "linux", "main")
        x86_64_dir = os.path.dirname(x86_64_path)
        cpp_dir = os.path.join(object_dir, "linux")
        ...
        if not os.path.exists(object_dir + "/build/linux"):
            os.makedirs(object_dir + "/build/linux")
        else:
            for file in os.listdir(object_dir + "/build/linux"):
                os.remove(os.path.join(object_dir + "/build/linux", file))
