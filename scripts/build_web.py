...
def build_web(project_dir):
    ...
    for file in os.listdir(os.path.join(project_dir, "web")):
        ...
        if os.path.isdir(os.path.join(project_dir, "web", file)):
            ...
            shutil.copy2(os.path.join(project_dir, "web", file), html_dir)

    if platform == "linux":
        build_modules(os.path.join(project_dir, "src"), html_dir, "python3")
        build_modules(os.path.join(project_dir, "lib"), html_dir, "python3")
    else:
        build_modules(os.path.join(project_dir, "src"), html_dir, "python")
    ...
    zip_folder(os.path.join(project_dir, "src"), os.path.join(html_dir, "src.zip"))
    zip_folder(os.path.join(project_dir, "lib"), os.path.join(html_dir, "lib.zip"))
