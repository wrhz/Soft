...
python_home = os.path.join(project_path, "build", "windows", "python")
pip_path = os.path.join(python_home, "Scripts", "pip.exe")
packages_path = os.path.join(project_path, "packages", "windows")
mode = sys.argv[1]
...
try:
    ...
    subprocess.run([pip_path, "freeze", ">", "softpm_requirements.txt"], cwd=project_path, shell=True)
except Exception as e:
